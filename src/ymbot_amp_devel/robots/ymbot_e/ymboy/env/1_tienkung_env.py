...
class TienKungEnv(VecEnv):
    ...
    #     lin_vel = visual_motion_frame[27:30].clone()  # 原始代码
    #     ang_vel = torch.zeros_like(lin_vel)  # 原始代码
    #     # lin_vel = torch.zeros(3, device=device)  # 修改为全零以测试是否影响可视化
    #     # ang_vel = visual_motion_frame[29:32].clone()  # 修改为随机数以测试角速度是否影响可视化
    ...
    def check_reset(self):
        ...
        # 检查机器人质心高度是否过低（新增的重置条件）
        # 当质心高度低于0.5米时，认为机器人已经摔倒，需要重置
        robot_height = self.robot.data.root_pos_w[:, 2]  # 获取z轴坐标（高度）
        height_reset = robot_height < self.cfg.robot.min_height_threshold
    ...
    def get_amp_obs_for_expert_trans(self):
        ...
        self.left_arm_local_vec = torch.tensor([0.0, 0.0, -0.0], device=self.device).repeat((self.num_envs, 1))
        self.right_arm_local_vec = torch.tensor([0.0, 0.0, -0.0], device=self.device).repeat((self.num_envs, 1))

        left_hand_pos = (
            self.robot.data.body_state_w[:, self.hand_body_ids[0], :3]
            - self.robot.data.root_state_w[:, 0:3]
            + quat_rotate(self.robot.data.body_state_w[:, self.hand_body_ids[0], 3:7], self.left_arm_local_vec)
        )
        right_hand_pos = (
            self.robot.data.body_state_w[:, self.hand_body_ids[1], :3]
            - self.robot.data.root_state_w[:, 0:3]
            + quat_rotate(self.robot.data.body_state_w[:, self.hand_body_ids[1], 3:7], self.right_arm_local_vec)
        )
