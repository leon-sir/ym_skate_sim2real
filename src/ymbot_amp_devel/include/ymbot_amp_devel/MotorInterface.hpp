...
namespace MotorInterface
{
    ...
    private:
        ...
        // 带去抖的扭矩超限计数，阈值，以及并联电机limit最小值（用于临时放宽保护时避免指数衰减到几乎0）
        int torqueExceedCount[JOINT_MOTOR_NUM] = {0};
        int torqueExceedThreshold = 3; // 需要连续超过阈值次数才判定为错误
        double torqueThresholdMultiplier = 2.0; // 将原来的1.5放宽为2.0
        double parallelMotorLimitMin = 0.2; // parallelMotorLimit 的下限
        int errorState{0};                                            //
