...
// 调试：打开文件用于写入数据
std::ofstream outfile("/home/pc/ymbot_e_13dof_skate/src/data/sim2real_rec_joint.txt", std::ios::out);
std::ofstream outfile_send("/home/pc/ymbot_e_13dof_skate/src/data/sim2real_send_joint.txt", std::ios::out);
std::ofstream onnx_rec("/home/pc/ymbot_e_13dof_skate/src/data/sim2real_onnx_rec_joint.txt", std::ios::out);
std::ofstream onnx_send("/home/pc/ymbot_e_13dof_skate/src/data/sim2real_onnx_send_joint.txt", std::ios::out);
std::ofstream motor_velocity("/home/pc/ymbot_e_13dof_skate/src/data/sim2real_rec_velocity.txt", std::ios::out);

// [新增] 重力投影对比日志文件
std::ofstream gravity_mocap_log("/home/pc/ymbot_e_13dof_skate/src/data/gravity_mocap.txt", std::ios::out);      // 动捕方式
std::ofstream gravity_imu_log("/home/pc/ymbot_e_13dof_skate/src/data/gravity_imu_calc.txt", std::ios::out);     // IMU计算方式
...
bool g_use_direct_vrpn = false;
std::unique_ptr<ymbot_amp_devel::DirectVrpnProcessor> g_direct_vrpn_processor;
...
std::tuple<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd> readProcessedMotionState()
{
    Eigen::VectorXd current_lin_vel(3);
    Eigen::VectorXd current_ang_vel(3);
    Eigen::VectorXd current_proj_grav(3);

    if (g_use_direct_vrpn && g_direct_vrpn_processor)
    {
        const auto state = g_direct_vrpn_processor->getState();
        current_lin_vel << state.base_linear_velocity.x(),
            state.base_linear_velocity.y(),
            state.base_linear_velocity.z();
        current_ang_vel << state.base_angular_velocity.x(),
            state.base_angular_velocity.y(),
            state.base_angular_velocity.z();
        current_proj_grav << state.projected_gravity.x(),
            state.projected_gravity.y(),
            state.projected_gravity.z();
        g_current_heading = state.heading;

        if (!state.has_pose)
        {
            ROS_WARN_THROTTLE(1.0, "Direct VRPN mode is enabled, but no VRPN pose has been received yet.");
        }

        return std::make_tuple(current_lin_vel, current_ang_vel, current_proj_grav);
    }

    std::lock_guard<std::mutex> lock(sensor_mutex);
    current_lin_vel = g_base_lin_vel;
    current_ang_vel = g_base_ang_vel;
    current_proj_grav = g_projected_gravity;
    return std::make_tuple(current_lin_vel, current_ang_vel, current_proj_grav);
}
...
// [方法1] 获取观测数据函数 - 动捕方式获取重力投影
// 返回值顺序: (关节位置, 关节速度, 四元数, 线速度, 角速度, 重力投影)
std::tuple<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd> get_observation_mocap(long long shared_timestamp)
{
    // 1. 读取下半身电机数据
    ...
    // 4. [修改核心] 读取处理后的动捕数据
    auto [current_lin_vel, current_ang_vel, current_proj_grav] = readProcessedMotionState();

    // [新增] 记录动捕方式的重力投影数据（带时间戳）
    if (gravity_mocap_log.is_open()) {
        gravity_mocap_log << shared_timestamp << "," 
                         << current_proj_grav[0] << "," 
                         << current_proj_grav[1] << "," 
                         << current_proj_grav[2] << "\n";
        gravity_mocap_log.flush();  // 强制刷新缓冲区，确保数据实时写入
    }
...
std::tuple<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd> get_observation_imu_calc(long long shared_timestamp)
{
    ...
    // 4. 读取处理后的线速度和角速度；重力投影继续使用 IMU 计算值
    Eigen::VectorXd current_lin_vel(3);
    Eigen::VectorXd current_ang_vel(3);
    Eigen::VectorXd unused_mocap_gravity(3);
    std::tie(current_lin_vel, current_ang_vel, unused_mocap_gravity) = readProcessedMotionState();
...
void run_real(const realcfg &real_cfg, AMPController &amp_controller)
{
    ...
    FootIndicesGenerator foot_indices_generator;
    ...
    while (ros::ok())
    {
        ...
        if (errorState == 0)
        {
            // [修改] 获取共享时间戳，确保两个观测函数使用相同的时间戳
            auto shared_timestamp = std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::system_clock::now().time_since_epoch()).count();
            
            // [修改] 获取机器人状态，解包新的返回值
            // 注意：第4个参数是 base_linear_velocity，第5个是 base_angular_velocity
            auto [q_Motor, dq_Motor, quat, base_linear_velocity, base_angular_velocity, projected_gravity] = get_observation(shared_timestamp);
            
            // [新增] 同时调用IMU计算方式，用于对比记录（不影响控制）
            auto [q_Motor_imu, dq_Motor_imu, quat_imu, base_linear_velocity_imu, base_angular_velocity_imu, projected_gravity_imu] = get_observation_imu_calc(shared_timestamp);
...
int main(int argc, char *argv[])
{
    ...
    {
        perror("shm_open");
        ROS_ERROR("Failed to open IMU shared memory. IMU data will not be available.");
        ...
        {
            perror("mmap");
            ROS_ERROR("Failed to map IMU shared memory. IMU data will not be available.");
            return 0;
        } else {
            ROS_INFO("IMU shared memory successfully opened and mapped.");
        }
    }
    ...
    nh.param("use_direct_vrpn", g_use_direct_vrpn, false);
    ...
    if (g_use_direct_vrpn)
    {
        g_direct_vrpn_processor = std::make_unique<ymbot_amp_devel::DirectVrpnProcessor>(nh);
        ROS_INFO("Using integrated direct VRPN pipeline inside amp_sim2real.");
    }
    else
    {
        sub_grav = nh.subscribe<geometry_msgs::Vector3Stamped>("/projected_gravity", 1, projectedGravityCallback);
        sub_vel = nh.subscribe<geometry_msgs::Vector3Stamped>("/projected_velocity", 1, projectedVelocityCallback);
        sub_omega = nh.subscribe<geometry_msgs::Vector3Stamped>("/projected_omega", 1, projectedOmegaCallback);
        sub_heading = nh.subscribe<std_msgs::Float32>("/current_heading", 1, currentHeadingCallback);
        ROS_INFO("Using legacy projected_* topic pipeline.");
    }
