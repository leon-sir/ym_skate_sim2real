    ...
    , configured_(false)
{
    ...
    // 数据缓冲区
    data_buffer_ptr_ =     std::shared_ptr<std::deque<char>>(new std::deque<char>(buffer_size_));
    // 读取串口数据所需的变量
    bytes_    = 0;
    ...
    deseralize_thread_ = std::thread( &YesenseDriver::_spin,this);
}
...
//         std::string file_path = "/home/ym/rlFdd/rlYm_fdd/rl_sar_zhixi/pythonPrj/data/dataIMU.txt";

//         // 打开文件
//         std::ofstream output_file(file_path, std::ios::out | std::ios::app);
...
//         else
//         {
//             ROS_INFO("Successfully opened file: %s", file_path.c_str());
//         }

//         while (ros::ok())
//         {
//             int available_bytes = serial_.available();
//             ROS_INFO("Available bytes: %d", available_bytes); // 监测串口数据是否可读

//             if (available_bytes > 0)
//             {
//                 data_ = serial_.read(available_bytes);
...
{
    std::shared_ptr<uint8_t[]>  buffer = std::shared_ptr<uint8_t[]> (new uint8_t[64]);
...
#define BIAS_EST_GET "\x59\x53\x4D\x08\x00\x51\xA6\x9D"
   ...
   std::string cmd = msg.get()->data; // 获取数据
