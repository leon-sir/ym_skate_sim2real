...
void EtherCAT_Run()
{
    ...
    EtherCAT_Command_Set(); // s

    ec_send_processdata();
    // receive
    wkc = ec_receive_processdata(EC_TIMEOUTRET); // end
    EtherCAT_Data_Get();
...
void EtherCAT_Command_Set()
{
    for (int slave = 0; slave < ec_slavecount; ++slave)
    {
        ...
        send_motor_ctrl_cmd(&Tx_Message[slave], 3, 3, 0, 0, 0, 0, 0);
        send_motor_ctrl_cmd(&Tx_Message[slave], 4, 4, 0, 0, 0, 0, 0);
        send_motor_ctrl_cmd(&Tx_Message[slave], 5, 5, 0, 0, 0, 0, 0);
        send_motor_ctrl_cmd(&Tx_Message[slave], 6, 6, 0, 0, 0, 0, 0);
        ...
        // send_motor_ctrl_cmd(&Tx_Message[slave], 6, 6, 0, 0, 0, 0, 0);
        // send_motor_ctrl_cmd(&Tx_Message[slave], 3,3, 0, 0, 0, 0, 0);
        // MotorIDSetting(&Tx_Message[0],1,8);

        // set_motor_position(&Tx_Message[0], 4,1, 0, 500, 600, 1);
        // MotorIDReset(&Tx_Message[0]);

        // set_motor_speed(&Tx_Message[slave], 1,2, 10, 30, 1);
        // set_motor_cur_tor(&Tx_Message[slave],1,1,300,0,1);
